#include "../include/circular_array_queue.h"
...
template <typename T>
std::size_t structures::CircularArrayQueue<T>::size(void) const {
  return size_;
}

template <typename T>
std::size_t structures::CircularArrayQueue<T>::max_size(void) const {
  return max_size_;
}
